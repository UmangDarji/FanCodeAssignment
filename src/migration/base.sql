...
create table newsarticle (
  ...
  primary key (id),
  key matchid_fk_idx (matchid),
  key tourid_fk_idx (tourid),
  key sports_fk_idx (sportsid),
  constraint matchid_fk foreign key (matchId) references matches (id),
  constraint sports_fk foreign key (sportsId) references sports (id),
  constraint tourid_fk foreign key (tourId) references tours (id)
);
